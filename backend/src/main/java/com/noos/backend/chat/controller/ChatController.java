 ...
 *   /app/chat.send    → 메시지 전송 (서버 수신)
 *   /app/chat.join    → 채팅방 입장
 *   /app/chat.leave   → 채팅방 퇴장
 *   /topic/room.{roomId} → 해당 방 구독자에게 브로드캐스트
 *
 * REST 엔드포인트:
...
public class ChatController {
    ...
    /**
     * 채팅 메시지 전송 핸들러
     * 클라이언트: stompClient.send("/app/chat.send", {}, JSON.stringify(message))
     * 브로드캐스트: /topic/room.{roomId} 구독자 전체에게 전달
     */
    @MessageMapping("/chat.send")
    public void sendMessage(@Payload ChatMessage message) {
        // 서버 측 타임스탬프 설정 (클라이언트 시간 신뢰 X)
